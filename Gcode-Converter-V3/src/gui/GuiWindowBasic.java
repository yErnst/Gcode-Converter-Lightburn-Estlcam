...
public class GuiWindowBasic extends JFrame {
	...
	public GuiWindowBasic(File explorerroot) {
		icon = new ImageIcon((Main.class.getResource("/images/Gcode-Converter.png")));
		this.explorerroot = explorerroot;
		this.setLocation(0, 0);
		this.setTitle("Gcode Converter " + Main.version + " by yErnst (Einige Funktionen nicht verfügbar. Benutze Windows mit Java 17 oder neuer)");
	...
	public void initComponents() {
		...
		Float[] spaceings = { 10f, 5f, 4f, 3f, 2.5f, 2f, 1f, 0.5f };
		GridSpaceing = new JComboBox<Float>(spaceings);
		...
		canvas = new Canvas(0, 60, this.getWidth() - 316, this.getHeight() - 108);
	...
	public void update() {
		openNCFile.setBounds(0, 30, 150, 30);
		InfoBar.setBounds(0, this.getHeight() - 68, this.getWidth() - 116, 30);
		menuBar.setBounds(0, 0, this.getWidth() - 16, 30);
		ShownLineCount.setBounds(150, 30, this.getWidth() - 166, 30);
		canvas.setSize(this.getWidth() - 316, this.getHeight() - canvas.getY() - InfoBar.getHeight() - 38);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (rv == JFileChooser.APPROVE_OPTION) {
					...
					if (!fc.getSelectedFile().getName().endsWith(".nc"))
						f = new File(fc.getSelectedFile().getAbsoluteFile() + ".nc");
					p.save(f);
				}
			}
		});
